...
class SemLaserScan(LaserScan):
  ...
  def __init__(self, nclasses, sem_color_dict=None, DATA=None, project=False, H=64, W=1024, fov_up=3.0, fov_down=-25.0):
  ...
  def set_label(self, label):
    ...
    if label.shape[0] == self.points.shape[0]:
      ...
      self.sem_label = self.remap_lut[self.label]
    ...
    # sanity check
    assert((self.label + (self.inst_label << 16) == label).all())
...
def scan_label_file(seq, index):
  base = f"/media/kitti/dataset/sequences/{seq}"
  new_index = ''.join(['0' for i in range(6 - len(str(index)))]) + str(index)
  return f'{base}/velodyne/{new_index}.bin', f'{base}/labels/{new_index}.label'
...
def transform(sequence):
  ...
  for i in range(num_files):
    scan_file, label_file = scan_label_file(sequence, i)
  ...
  np.save(f'{path}/range', scans_range)
